...
class WeatherApp {
  ...
  setupEventListeners() {
    try {
      ...
      this.elements.searchDropdown.addEventListener(
        "click",
      ...
      this.elements.searchDropdown.addEventListener(
        "keydown",
        this.handleDropdownKeyPress.bind(this)
      );
  ...
  updateCurrentTime() {
    try {
      ...
      if (this.elements.currentDate)
        this.elements.currentDate.setAttribute(
          "datetime",
  ...
  displayRandomFact() {
    try {
      const randomFact =
        this.weatherFacts[Math.floor(Math.random() * this.weatherFacts.length)];
      const randomDesc =
        this.locationDescriptions[
          Math.floor(Math.random() * this.locationDescriptions.length)
        ];

      if (this.elements.loadingFact)
        this.elements.loadingFact.textContent = randomFact;
      if (this.elements.loadingDescription)
        this.elements.loadingDescription.textContent = randomDesc;
    } catch (error) {
  ...
  handleKeyPress(event) {
    if (event.key === "Enter") {
      ...
      event.preventDefault();
      const firstResult =
        this.elements.searchDropdown.querySelector("[data-lat]");
      if (firstResult) firstResult.focus();
    }
  }

  handleDropdownKeyPress(event) {
    const items = this.elements.searchDropdown.querySelectorAll("[data-lat]");
  ...
  handleDropdownClick(event) {
    ...
    if (cityDiv && !this.isLoading) {
      ...
      this.elements.searchInput.focus(); // Return focus
    }
  }
  ...
  displaySearchResults(cities) {
    const html = cities
      .map((city) => {
        ...
        const escapedLocation = this.escapeHtml(location); // Escape for data attribute

        return `
                <div class="search-item" role="option" tabindex="0" data-lat="${
                  city.latitude
                }" data-lon="${city.longitude}" data-name="${escapedLocation}">
                    <span class="text-xl" aria-hidden="true">${flag}</span>
                    <div class="flex-1">
                        <div class="font-semibold text-white">${this.escapeHtml(
                          city.name
                        )}</div>
                        <div class="text-sm text-gray-400">
                            ${this.escapeHtml(
                              city.admin1 ? city.admin1 + ", " : ""
                            )}${this.escapeHtml(city.country)}
                        </div>
                    </div>
                </div>
            `;
      ...
      return {
        ...
        current: {
          ...
          apparentTemperature: Math.round(
            data.current?.apparent_temperature || 20
          ),
        ...
        hourly: {
          ...
          temperature: (data.hourly?.temperature_2m?.slice(1, 13) || []).map(
            (t) => Math.round(t)
          ),
          precipitationProbability:
            data.hourly?.precipitation_probability?.slice(1, 13) || [],
        ...
        daily: {
          ...
          temperatureMin: (
            data.daily?.temperature_2m_min?.slice(0, 7) || []
          ).map((t) => Math.round(t)),
          ...
          precipitationProbabilityMax:
            data.daily?.precipitation_probability_max?.slice(0, 7) || [],
        },
      };
    } catch (error) {
  ...
  handleWeatherError(error, locationName) {
    ...
    const heroCard = document.querySelector(".neon-card-blue");
    if (heroCard) {
      heroCard.classList.add("error-state");
      heroCard.setAttribute("aria-label", "Using cached weather data");
      setTimeout(() => {
        heroCard.classList.remove("error-state");
    ...
    return {
      ...
      hourly: {
        ...
        temperature: Array.from({ length: 12 }, (_, i) => {
          ...
          return Math.round(
            baseTemp + tempVariation + (Math.random() - 0.5) * 4
          );
        }),
        precipitationProbability: Array.from({ length: 12 }, () =>
      ...
      daily: {
        ...
        temperatureMax: Array.from({ length: 7 }, () =>
          Math.round(baseTemp + 3 + Math.random() * 8)
        ),
        temperatureMin: Array.from({ length: 7 }, () =>
          Math.round(baseTemp - 8 + Math.random() * 6)
        ),
        weatherCode: Array.from({ length: 7 }, () =>
          Math.random() > 0.6 ? 61 : 1
        ),
        ...
        uvIndexMax: Array.from({ length: 7 }, () =>
          Math.round(3 + Math.random() * 8)
        ),
        precipitationSum: Array.from({ length: 7 }, () => Math.random() * 15),
        precipitationProbabilityMax: Array.from({ length: 7 }, () =>
          Math.round(Math.random() * 80)
        ),
      },
    };
  ...
  updateHeroWeather() {
    ...
    this.updateElementText(
      "weatherCondition",
      this.getWeatherCondition(current.weatherCode)
    );
    this.updateElementText(
      "mainWeatherIcon",
      this.getWeatherIcon(current.weatherCode, current.isDay)
    );
  ...
  updateWindInfo() {
    ...
    this.updateElementText(
      "windDirection",
      this.getWindDirection(current.windDirection)
    );
  ...
  updateUVAir() {
    ...
    const maxUV = Math.max(
      ...this.weatherData.hourly.uvIndex.slice(6, 18).filter((uv) => uv > 0)
    );
  ...
  updateHourlyForecastAndInsights() {
    ...
    const html = hourly.temperature
      .map((temp, i) => {
        ...
        const timeLabel =
          i === 0 ? "Now" : `${String(hour).padStart(2, "0")}:00`;
  ...
  generateWeatherInsights(current) {
    ...
    if (current.temperature > 35) {
      ...
      insights.push(
        "🌞 Hot weather perfect for water activities. Don't forget sunscreen!"
      );
    } else if (current.temperature < 5) {
      insights.push(
        "🧊 Freezing conditions! Layer up and watch for icy surfaces."
      );
    } else if (current.temperature < 15) {
      insights.push(
        "☕ Cool weather ideal for cozy indoor activities or warm beverages."
      );
    } else if (current.temperature >= 20 && current.temperature <= 28) {
      insights.push(
        "🌈 Perfect weather conditions! Great day for any outdoor plans."
      );
    }

    if (current.humidity > 85) {
      insights.push(
        "💧 Very humid - air conditioning recommended for comfort."
      );
    ...
    if (current.precipitation > 5) {
      insights.push(
        "📖 Heavy rain expected - perfect weather for indoor activities!"
      );
    }

    if (insights.length === 0) {
      insights.push(
        "😊 Beautiful conditions ahead! Enjoy your day to the fullest!"
      );
    }
  ...
  updateWeeklyForecast() {
    ...
    const html = daily.temperatureMax
      .slice(0, 6)
      .map((maxTemp, i) => {
        const date = new Date(today);
        date.setDate(date.getDate() + i);
        const dayName =
          i === 0 ? "Today" : i === 1 ? "Tomorrow" : dayNames[date.getDay()];
